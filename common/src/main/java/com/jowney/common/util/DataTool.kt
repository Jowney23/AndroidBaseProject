...
import com.jowney.common.util.ConstantsTool.BYTE
import com.jowney.common.util.ConstantsTool.GB
import com.jowney.common.util.ConstantsTool.KB
import com.jowney.common.util.ConstantsTool.MB
import com.jowney.common.util.ConstantsTool.SPACE
import java.io.*
...
class DataTool {
    companion object {
         ...
         */
        @JvmStatic
        fun byte2Size(byteNum: Long, unit: ConstantsTool.MemoryUnit?): Double {
            return if (byteNum < 0) {
                (-1).toDouble()
            } else when (unit) {
                ConstantsTool.MemoryUnit.BYTE -> byteNum.toDouble() / BYTE
                ConstantsTool.MemoryUnit.KB -> byteNum.toDouble() / KB
                ConstantsTool.MemoryUnit.MB -> byteNum.toDouble() / MB
                ConstantsTool.MemoryUnit.GB -> byteNum.toDouble() / GB
         ...
         */
        @JvmStatic
        fun size2Byte(size: Long, unit: ConstantsTool.MemoryUnit?): Long {
            return if (size < 0) {
                -1
            } else when (unit) {
                ConstantsTool.MemoryUnit.BYTE -> size * BYTE
                ConstantsTool.MemoryUnit.KB -> size * KB
                ConstantsTool.MemoryUnit.MB -> size * MB
                ConstantsTool.MemoryUnit.GB -> size * GB
                else -> size * BYTE
            }
        }
        ...
        fun changeDistance(length: Double, displayMeter: Boolean): String {
            return if (length < 1000) {
                ConstantsTool.FORMAT_TWO.format(length) + if (displayMeter) "米" else ""
            } else {
                ConstantsTool.FORMAT_TWO.format(length / 1000) + if (displayMeter) "千米" else ""
            }
        }
